...
### Databases
- **`erp.db`** — Business data (SQLAlchemy, synchronous): `Articolo`, `Cliente`, `Ordine`, `RigaOrdine`, `Fornitore`, `CatalogoFornitore`
...
2. **Web Admin** (`erpclaw/web.py`): FastAPI + SQLAdmin CRUD for all ERP models.
...
The `team` object (agno `Team`) is the main entry point. It holds `ERPTools` and can delegate to `fornitore_research_agent` for supplier web research.
...
- `erpclaw/erp_db.py` — SQLAlchemy models and `get_session()` / `init_db()`. `init_db()` is called at import time in both `erp_tools.py` and `web.py`.
- `erpclaw/erp_tools.py` — `ERPTools(Toolkit)`: all agent tools. Tools return markdown strings.
...
- `erpclaw/agent.py` — `team` (agno `Team`) + `fornitore_research_agent` sub-agent + `memory_manager`. The `Team` is the entry point; it holds `ERPTools`, `db`, memory, and delegates to `fornitore_research_agent`. Memory manager uses `deepseek-chat`; both team leader and sub-agent use `deepseek-reasoner`.
